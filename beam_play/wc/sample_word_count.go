...
var (
	...
	output = flag.String("output", "outputs", "Output file (required).")
)
...
func CountWords(s beam.Scope, lines beam.PCollection) beam.PCollection {
	...
	debug.Printf(s, "inp: %v", debug.Head(s, lines, 10))

	// Convert lines of text into individual words.
	col := beam.ParDo(s, &extractFn{SmallWordLength: *smallWordLength}, lines)

	debug.Printf(s, "step1: %v", debug.Head(s, col, 10))

	// Convert toLowerCase
	lowerCaseWords := beam.ParDo(s, strings.ToLower, col)

	debug.Printf(s, "step2: %v", debug.Head(s, lowerCaseWords, 10))

	// Count the number of times each word occurs.
	res := stats.Count(s, lowerCaseWords)

	debug.Printf(s, "out: %v", debug.Head(s, res, 10))
